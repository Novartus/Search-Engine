...
public class RankingWebPages {
	...
	public List<Entry<Integer, Integer>> sortingText(String singleWord) {
		...
		for (Entry<Integer, Integer> entry : inpuText)
	...
	private List<Entry<Integer, Integer>> sortingHashMap(HashMap<Integer, Integer> trtm) {
		List<Entry<Integer, Integer>> lst = new ArrayList<>(trtm.entrySet());
		Collections.sort(lst, new Comparator<Entry<Integer, Integer>>() {
			public int compare(Entry<Integer, Integer> text1, Entry<Integer, Integer> text2) {
